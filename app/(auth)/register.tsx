        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="none">
            ...
            <Animated.View entering={FadeInDown.delay(60).duration(450)} style={s.logoRow}>
              <BrandLogo size={32} />
              <Text style={s.logoName}>AgriNex</Text>
            </Animated.View>

            <Animated.View entering={FadeInDown.delay(120).duration(450)} style={s.header}>
              <Text style={s.title}>Join AgriNex</Text>
              <Text style={s.subtitle}>Create your smart farming account</Text>
            </Animated.View>

            <Animated.View entering={FadeInDown.delay(200).duration(450)} style={s.form}>
              <Pressable onPress={() => fullNameRef.current?.focus()} style={[s.inputWrap, focusedField === 'fullName' && s.inputFocus]}>
                <User color={focusedField === 'fullName' ? PRIMARY : 'rgba(255,255,255,0.35)'} size={18} style={s.inputIcon} />
                <TextInput ref={fullNameRef} style={s.input} placeholder="Full Name" placeholderTextColor="rgba(255,255,255,0.3)" autoCapitalize="words" value={fullName} onChangeText={setFullName} onFocus={() => setFocusedField('fullName')} onBlur={() => setFocusedField(null)} autoCorrect={false} onSubmitEditing={() => emailRef.current?.focus()} returnKeyType="next" />
              </Pressable>

              <Pressable onPress={() => emailRef.current?.focus()} style={[s.inputWrap, focusedField === 'email' && s.inputFocus]}>
                <Mail color={focusedField === 'email' ? PRIMARY : 'rgba(255,255,255,0.35)'} size={18} style={s.inputIcon} />
                <TextInput ref={emailRef} style={s.input} placeholder="Email address" placeholderTextColor="rgba(255,255,255,0.3)" keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail} onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} autoCorrect={false} onSubmitEditing={handleContinue} returnKeyType="done" />
              </Pressable>
            </Animated.View>
...
const s = StyleSheet.create({
  ...
  backBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.06)', justifyContent: 'center', alignItems: 'center', marginBottom: 20 },
  ...
  btnShadow: { width: '100%', shadowColor: PRIMARY, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.25, shadowRadius: 12, elevation: 4 },
